...
impl Frame for TrackList {
    ...
    fn draw(ui: &mut egui::Ui, app: &mut MidiApp) {
        ...
        TrackList::header(ui, TrackList::FRAME_NAME);
        ...
        TableBuilder::new(ui)
            ...
            .body(|mut body| TrackList::set_track(&mut body, app));
    }
}

impl TrackList {
    fn set_track(body: &mut egui_extras::TableBody<'_>, app: &mut MidiApp) {
        let mut midi_manager = app.midi_manager.lock().unwrap();
        if !midi_manager.is_loaded() { return; }

        // println!("Track: {:?}", self.smf.as_ref().unwrap());
        // for track in app.midi_manager.as_mut().unwrap().tracks.iter_mut() {
            // for event in track.iter_mut(){
            //     println!("{:?}", event);
            // }

            // let is_selected = self.selected_track_id == Some(track);
